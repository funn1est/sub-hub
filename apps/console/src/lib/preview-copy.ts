...
function formatExpire(expire: number, locale: Locale): string {
  return new Date(expire * 1000).toLocaleDateString(locale === 'zh' ? 'zh-CN' : 'en-US', {
    dateStyle: 'medium',
  });
}
...
export function trafficSummary(locale: Locale, traffic: SubscriptionUserInfo): string {
  ...
  if (traffic.total > 0) {
    ...
    return locale === 'zh' ? `已用 ${used}，共 ${cap}` : `${used} used of ${cap}`;
  }
  return locale === 'zh'
...
export function skippedSummary(locale: Locale, counts: SkipCounts): string {
  ...
  if (counts.parse > 0) {
    parts.push(
      locale === 'zh' ? `${counts.parse} 个读不出来` : `${counts.parse} could not be read`,
    );
  }
...
function previewError(locale: Locale, preview: PreviewDone): PreviewProfile['error'] {
...
export function previewProfile(locale: Locale, preview: PreviewDone): PreviewProfile {
  return {
    ...
    skipped: preview.skipped !== null ? skippedSummary(locale, preview.skipped) : undefined,
    omitted:
