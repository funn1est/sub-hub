...
describe('static headers and PWA', () => {
  ...
  it('does not long-cache HTML; hashed assets are immutable', () => {
    ...
    expect(blocks.get('/assets/*')).toEqual(['Cache-Control: public, max-age=31536000, immutable']);
  ...
  it('asks crawlers not to index workers.dev hostnames', () => {
    ...
    expect(blocks.get('https://:name.:subdomain.workers.dev/*')).toEqual(noindex);
    expect(blocks.get('https://:version.:name.:subdomain.workers.dev/*')).toEqual(noindex);
  });

  it('does not navigate-fallback Conversion Service paths', () => {
    const text = readFileSync(resolve(import.meta.dirname, '../../vite.config.ts'), 'utf8');
