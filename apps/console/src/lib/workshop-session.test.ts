...
describe('createWorkshopSession', () => {
  ...
  it('resets Preview when fields change and drops the stale in-flight result', async () => {
    ...
    const { session, view } = makeSession({
      fields: { serviceOrigin: ORIGIN },
      fetch: (url) => (url.endsWith('/version') ? response(200, VERSION_OK) : gate.promise),
    });
  ...
  it('walks config selection across none, preset, and custom', () => {
    ...
    session.actions.editCustomConfigUrl(acl4ssrConfigUrl('ACL4SSR_Online_Mini.ini'));
  ...
  it('probes /version for the canonical origin and drops stale probes', async () => {
    ...
    const { session, view, calls } = makeSession({
      fetch: (url) => (url === `${ORIGIN}/version` ? first.promise : response(200, VERSION_OK)),
    });
    ...
    expect(calls).toEqual([`${ORIGIN}/version`, 'https://other.example/version']);
  ...
  it('ignores a late same-origin guess after the origin field is filled', async () => {
    ...
    const { session, view, calls } = makeSession({
      fields: { serviceOrigin: '' },
      consoleOrigin: ORIGIN,
      fetch: (url) => (url === `${ORIGIN}/version` ? gate.promise : response(200, VERSION_OK)),
    });
    session.actions.patch({ serviceOrigin: 'https://other.example' });
  ...
  it('notifies copied or copy-failed from the clipboard port', async () => {
    ...
    expect(wrote).toEqual([`${ORIGIN}/sub?target=clash&url=${VLESS_ENCODED}&expand=true`]);
    ...
    wrote.length = 0;
    const quanx = sibling.view().assembled.siblings.find((item) => item.target === 'quanx');
    expect(quanx?.url).toBe(`${ORIGIN}/sub?target=quanx&url=${VLESS_ENCODED}&expand=true`);
    await sibling.session.actions.copy(quanx?.url);
    expect(wrote).toEqual([`${ORIGIN}/sub?target=quanx&url=${VLESS_ENCODED}&expand=true`]);
  ...
  it('pastes clipboard text into the source list using the row-split rules', async () => {
    ...
    const conversion = 'http://127.0.0.1:25500/sub?target=clash&url=vless%3A%2F%2Fa';
