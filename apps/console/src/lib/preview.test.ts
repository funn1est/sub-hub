...
describe('truncatePreviewBody', () => {
  it('keeps bodies at the view cap and marks overflow', () => {
    ...
    expect(new TextEncoder().encode(truncated.text).length).toBe(PREVIEW_VIEW_LIMIT_BYTES);
...
describe('download filename', () => {
  it('prefers content-disposition and falls back to sub-hub-<target>.<ext>', () => {
    expect(filenameFromDisposition('attachment; filename="sub-hub-mihomo.yaml"')).toBe(
      'sub-hub-mihomo.yaml',
    );
  ...
  it('returns a done Preview from the Subscription URL GET', async () => {
    const outcome = await runPreview({
      ...
      fetchImpl: async () => ({
        ...
        headers: {
          get: (name: string) =>
            name === 'content-disposition' ? 'attachment; filename="sub-hub-mihomo.yaml"' : null,
        },
      }),
    });
  ...
  it('parses subscription-userinfo from the same GET', async () => {
    ...
    const outcome = await runPreview({
      ...
      fetchImpl: async () => ({
        ...
        headers: {
          get: (name: string) =>
            name === 'subscription-userinfo' ? 'upload=1; download=2; total=3; expire=0' : null,
        },
      }),
    });
    expect(outcome.status).toBe('done');
