...
export function readSubGetHeaders(
  ...
  return {
    ...
    filename:
      filenameFromDisposition(headers.get('content-disposition')) ?? fallbackDownloadName(target),
    exposed,
  };
}
...
export type VersionState = { status: 'idle' } | { status: 'checking' } | VersionProbe;
...
export async function runVersionProbe(input: {
  ...
  try {
    const response = await (input.fetchImpl ?? fetch)(`${input.origin}${VERSION_PATH}`, {
      signal: input.signal,
    });
...
export type PreviewOutcome = PreviewDone | { status: 'unreachable'; cause: FetchFailure };

export type PreviewState = { status: 'idle' } | { status: 'loading' } | PreviewOutcome;
