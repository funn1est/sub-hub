...
import { ACL4SSR_FAMILIES, ACL4SSR_PRESETS, acl4ssrListed } from './acl4ssr-catalog.ts';
import { messages } from './i18n.ts';

describe('ACL4SSR_PRESETS', () => {
  it('groups INIs by family in combobox order', () => {
    expect(Object.keys(ACL4SSR_PRESETS)).toEqual(['online', 'mini', 'full', 'classic']);
  ...
  it('lists every INI once, with Online.ini as ads and China split', () => {
    ...
    expect(
      ACL4SSR_PRESETS.online.find((preset) => preset.file === 'ACL4SSR_Online.ini'),
    ).toMatchObject({ effect: 'adsChinaSplit' });
    expect(listed.find((preset) => preset.file === 'ACL4SSR_Online_NoAuto.ini')).toMatchObject({
      effect: 'noAuto',
    });
    expect(
      listed.find((preset) => preset.file === 'ACL4SSR_Online_Full_Netflix.ini'),
    ).toMatchObject({ effect: 'netflix' });
    expect(
      listed.find((preset) => preset.file === 'ACL4SSR_NoAuto_NoApple_NoMicrosoft.ini'),
